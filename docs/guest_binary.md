...
are not virtual methods either. They are entries in a **name → function binding
table at `0x8203F2E0`** — 228 pairs of `const char*` and code pointer, registered
from `sub_824F1C98` (site `0x824F1E1C`). The vocabulary is a scripting API.
...
A **second `(name, func)` table** lives in `.data` around **`0x82D1B21C`**,
distinct from the 228-entry one. It holds the VariableCollection bindings:
`sub_824B1C20` is `GetVariableString` — it validates its arguments through the
same `sub_82A9F4F8` reporter as `ExecuteScriptAsset` and names itself
`VariableCollection_GetVariableString` in its own error strings — and
`sub_824B1788` is the int twin, `GetVariableInt`.
