...
bool BinkPlayer::Open(const std::string& path) {
  Close();
...
bool BinkPlayer::DecodeNextFrame(std::vector<uint8_t>& outRgba) {
...
size_t BinkPlayer::ReadAudioSamples(float* dst, size_t maxSamples) {
  std::lock_guard<std::mutex> lock(m_audioMutex);
