...
// selector -- EmitShaderHlsl turns it back into an identity scale and applies
// the piecewise-linear curve instead.
...
void ReportAddGameDrawsCost(uint64_t microseconds, uint32_t draws);
 ...
 private:
  ...
  // mode reaches the renderer on HleTexturePayload and used to be discarded in
  // favour of one static WRAP sampler.
  //
  //   kSamplerPoint    the guest's own min/mag filter, which was thrown away --
  //                    every texture was MIN_MAG_MIP_LINEAR whatever the guest
  //                    asked for. A UI font atlas blitted one texel to one pixel
  //                    asks for point, and filtering it linearly blends each
  //                    glyph edge with its neighbour: the smeared menu text.
  ...
  // A shader-visible sampler heap is capped at 2048 descriptors, which sizes
  // everything below. The reserved region at the front holds the
  ...
  // Frame internals. BeginFrame picks between the two Render* and EndFrame calls
  ...
  // Uploads the Bink plane set into reusable host textures and writes their SRVs
  ...
  // emitter declares its textures contiguously from t0 up to that cap, and a
  // table narrower than the registers a shader declares is invalid.
  // static_asserted in d3d12_game.cpp.
  //
  ...
  // states. The bytecode is cached per handle either side of this, so a new key
  // past first sight costs one CreateGraphicsPipelineState, not a compile.
  ...
  // draw for the rest of the run. `draw` supplies the vertex stage: its
  // translated VS source and the register list the input layout is built from.
  ...
  // translated draw gets a fresh block here and its views are created directly
  // into it (D3D12 forbids CopyDescriptors from a shader-visible heap).
  //
  // Never rewritten within a frame, because overwriting a descriptor the GPU may
  // still be reading is undefined -- it read as zero on this hardware rather
  // than failing, which is how the video-plane block was found.
  //
  // PARTITIONED BY FRAME IN FLIGHT: frame f owns
  ...
  // one never submitted. `budget` is the one that can bite silently.
  //
  ...
  // the two agree on extent and format. On the console those are the same
  ...
  // SRV slots returned by evicted textures, safe to hand out again. Without this
  ...
  // to account for the growth is ruled out rather than argued about.
  ...
  // stand-in: a blank source with no candidate at its EDRAM base is a different
  // defect from one whose candidates were all blank too.
  ...
  // Why a banded DEPTH resolve did not stitch. BLANK-SOURCE alone says a resolve
  // copied an undrawn surface; it cannot say whether the stitch was unavailable,
  // mis-shaped, or merely not drawn yet, and those are three different repairs.
  // Keyed by reason, holding the first sighting's numbers.
  ...
  // reasons. m_depthBandResolves counts depth resolves satisfied by stitching
  // EDRAM bands rather than by an object-identity hit; non-zero means the shadow
  // pass is reaching its shader.
  ...
  // so that slot's copy is complete and mapping it cannot stall. The cost is a
  // couple of frames of latency on a value the guest filters over time anyway.
  //
  // kLuminanceReadbackBytes is generous enough that GetCopyableFootprints for
  // any single small texel can never need more, and a round multiple of the
  // 512-byte placement alignment.
  ...
  // SMALL DESTINATIONS THE GUEST READS FROM MEMORY -- the terrain's
  ...
  // was refused on every one of ~1880 attempts a run -- which looked exactly
  // like the floating bike's cause. All three halves of what happened next are
  // worth keeping:
  //
  ...
  //     ONE host site, r0 w80 -- our own writeback and nothing else. The one GPU
  //     consumer binds the host SNAPSHOT, so the guest-memory copy has no reader.
  ...
  //
  //     The 64x64 is the VT feedback buffer that drives tile streaming, and the
  //     near ground went black in the same build.
  //
  ...
  struct GameRenderTarget {
    ...
    // does not write depth (Z_WRITE off), so a copy is as good as sharing. Zero
    // owner means "not a band", which is every other surface.
  ...
  // buffer that broke every stencil light volume. Entries are revalidated on use
  // rather than invalidated on retire, so an owner going away degrades to a
