...
enum class AluStatus {
  ...
  //
  // Expect ~0 for this game: kUnsupportedCf is 0 across every sample, so the two
  // counts cross-check each other -- a non-zero kLoopRelative with a zero
  // kUnsupportedCf is not a shader that loops, it is a decode error, and that
  // combination is precisely what an inverted a0/aL selector looked like.
  kLoopRelative,
  kInstructionCap,
};
...
struct AluResult {
  ...
  // range says *which* slots it wanted. Counters rather than a callback, so this
  // header stays SDK-free; min > max means no constant was read at all.
...
// then reads. Does not throw and does not loop unbounded.
