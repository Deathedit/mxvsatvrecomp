...
//
// The renderer manufactures a plausible value wherever it cannot obtain the real
// one. Before any of that can be turned off, each site has to say how often it
// fires AND out of how many opportunities.
//
...
// Class B sites from docs/strict_mode.md: machinery that INVENTS output. Class A
// (HostPageReadable, PlausibleGuestPtr -- refusing to act on untrusted input) is
// deliberately absent; it models reality and is not the subject.
//
...
//
//   kStandInDraw          the draw is skipped -- it disappears, a diagnostic
...
//
//   kScratchColourTarget  every PSO declares NumRenderTargets = 1, so binding no
//                         RTV is invalid work, not a diagnostic
...
//                         initialiser, emitted by FXC
//   kMaterialGateFill     already inert; only the measurement remains
//
// Strict(g) is false for all four, always.
