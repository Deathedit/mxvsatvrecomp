...
| `debug_native` | — | Flips `sub_829E8FA8` (`return 0`) to 1, **per call site**. That address is the linker-folded body of *every* `return 0` in the image — Lua's `print` included — so its call sites are stubs generally, not debug checks; `all` returns 1 to callers expecting 0 and segfaults. `census` lists sites without changing anything; decompile a site before flipping it. `0x82AB6638` is verified as the DebugOverlay's enable check |
